...
const MAX_INLINE_URL_BYTES = {
  avatarUrl: 512 * 1024,
  coverUrl: 1024 * 1024,
  mediaUrl: 10 * 1024 * 1024, // Increased to 10MB for high-res Base64 images
};
...
// GET CURRENT USER PROFILE & POSTS
    ...
    const [itemsResult, postsResult] = await Promise.allSettled([
      ...
      (async () => {
        try {
          ...
          return [[], []]; // Return empty if post table fails
        }
      })(),
    ]);
  ...
  try {
    ...
    for (const [type, values] of Object.entries(profileItems)) {
      ...
      if (cleanValues.length) {
        const inserts = cleanValues.map((val) => [req.user.userId, type, val]);
...
router.put('/posts/:id/like', async (req, res) => {
  ...
  try {
    ...
    return res.json({ likes: post.likes, liked: existing.length === 0 });
  } catch (err) {
    await connection.rollback();
    return res.status(500).json({ message: 'Server error' });
  } finally {
